...
const Sample_code_PHP = () => {
  const [t, i18n] = useTranslation();
  const { language } = i18n;
  return (
    <>
      <div dir={language == "ar" ? "rtl" : "ltr"} className={`fs-5 text-dark ${styles.requirement}`}>
        <LinkHierarchy parent={t("docs.Rest_APIs.Rest_APIs")} child={t("docs.Sample_Code_PHP.Sample_Code_PHP")} />
        <MainTitleNew title={t("docs.Sample_Code_PHP.Sample_Code_PHP")} />
        <div dir={language == "ar" ? "ltr" : "rtl"}>
        <CodeSnippetCopy
          title="Create Rest API Invoices Iso - PHP"
          code={
            `<!DOCTYPE html>
<html lang="en">
<head>
<title>Safqa Payment Demo</title>
<meta charset="utf-8">
  ...
  <div className="row">
   <div className="col-md-6 col-sm-6 col-xs-12 text-center">
     <div className="form-group ">
      <label className="control-label " for="text">
       TEST API URL:
           https://apidemo.Safqa.com/swagger/ui/index#/ApiInvoices
      </label>
      <label className="control-label " for="text">
        Test Account: demoApiuser@Safqa.com<br>
     ...
     <div className="form-group ">
      ...
      <input className="form-control" id="username" name="username" placeholder="username" type="text" required/>
     </div>
     <div className="form-group ">
      <label className="control-label requiredField" for="name">
       Enter Password
      ...
      <input className="form-control" id="password" name="password" placeholder="password" type="passsword" required/>
     </div>
     <div className="form-group">
      <div>
       <button className="btn btn-primary " name="SubmitButton" type="submit">
      Generate Token
       </button>
      </div>
     </div>
    ...
    <br>
    <br>
    <a href="index.php"><button type="button" id=""  className="btn btn-success">Click here to Test Payment Link</button></a>
  </div>
<div className="col-md-6 col-sm-6 col-xs-12 text-center">
    <?php
if(isset($_POST['SubmitButton'])){
  ...
  if(empty($_POST['username']) || empty($_POST['password']))
    {
    echo "You did not fill out the requireds.";
    die();  // Note this
  }else{
      ...
      $curl = curl_init();
      curl_setopt($curl, CURLOPT_URL,'https://apidemo.Safqa.com/Token');
  ...
  "ErrorUrl": "http://www.bing.com"
        ...
        $soap_do     = curl_init();
       curl_setopt($soap_do, CURLOPT_URL, "https://apidemo.Safqa.com/ApiInvoices/CreateInvoiceIso");
      ...
      $referenceId =  $ref_Ex[4];
       curl_close($soap_do);
    echo '<br><button type="button" id="paymentRedirect"  className="btn btn-success">Click here to Payment Link</button>';
    ...
    $("#paymentRedirect").click(function(e) {
      ...
      if(isset($_GET['paymentId'])){
        ...
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, false);
        curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query(array('grant_type' => 'password','username' => 'demoApiuser@Safqa.com','password' => 'Mf@12345678')));
        $result = curl_exec($curl);
       ...
       if(isset($_GET['paymentId']))
        ...
        $password= 'api12345*';
        $url = 'https://apidemo.Safqa.com/ApiInvoices/Transaction/'.$id;
        $soap_do1 = curl_init();
        ...
        echo'<div className="form-group "><label className="control-label" for="name">
...
export default Sample_code_PHP
