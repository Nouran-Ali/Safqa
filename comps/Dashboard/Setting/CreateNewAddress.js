...
import { createAddress, ResetSuccess } from "../../../store/slices/addressSlice";
...
const CreateNewAddress = () => {
  ...
  const { addressInfo, isLoading, api_errors, success } = useSelector(
    (state) => state.address
  );
  ...
  return (
    <>
      <form
        ...
        className={`${styles.CreateNewAdresse} ${language == "ar" && styles.CreateNewAdresseAR + " me-5"}`}
      ...
      >
        ...
        <div className="text-center mt-4 mb-4">
          <MagicBtn isLoading={isLoading} label={t("dashboard.create")} />
        </div>
      </form>
    </>
  );
