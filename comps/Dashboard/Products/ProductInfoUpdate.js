...
import { createProductSchema, updateProductSchema } from "../../../lib/validations/en/productSchema";
import { useRouter } from 'next/router'
import { useTranslation } from "react-i18next";
import { useTheme } from "next-themes";
import { updateProductSchemaAr } from "../../../lib/validations/ar/productSchemaAr";

const ProductInfoUpdate = ({ product }) => {
  ...
  const {
    ...
    resolver: yupResolver(language == 'en' ? updateProductSchema : updateProductSchemaAr),
    defaultValues,
  });
  ...
  return (
    <form className={`mt-2 mb-4`} onSubmit={handleSubmit(onSubmit)}>
      <div className={`rounded-2 ${theme == 'dark' ? styles.info_dark : styles.info}`}>
        <p className="px-4 fs-5">Product info</p>
        <hr />
