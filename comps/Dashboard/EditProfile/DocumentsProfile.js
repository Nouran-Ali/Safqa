...
const AddDocument = ({ label, name, error, inputs, setValue }) => {
  const { theme } = useTheme();
  const [file, setFile] = useState("");
  const { profile_business } = useSelector((state) => state.profileBusiness);

  const { documentInfo, documents, urlFile, isLoading, api_errors, success } =
  ...
  return (
    <div className="row ms-3 mt-4">
      <label
        ...
        ${theme == "dark" ? styles.plus_dark : styles.plus} ${
          inputs[name]
            ? theme == "dark"
              ? styles.fileLabel_dark
              : styles.fileLabel
            : ""
        }`}
      >
      ...
      <div className="col mt-2 me-5 d-flex flex-column">
        <span className={`${theme == "dark" ? "text-grey" : "text-dark"} fs-5`}>
          {label}
        </span>

        <p
          className={`form-control border-0 shadow-none p-0 ${
            theme == "dark" ? styles.inpfile_dark : styles.inpfile
          }`}
        >
          {inputs[name]
            ? inputs[name]?.[0]?.name
        ...
        {!profile_business?.is_approval && (
          <input hidden type="file" id={label} onChange={onChangeFile} />
        )}
      </div>
      ...
      {documents?.[name] && (
        ...
        //     <BtnShow href={documents[name]} />
        //   </div>

        //   <div className="me-2">
        //     <BtnDownload url={documents[name]} />
        //   </div>
...
const DocumentsProfile = () => {
  ...
  const { documentInfo, documents, isLoading, api_errors, success } =
    useSelector((state) => state.document);
  ...
  const {
    ...
    resolver: yupResolver(
      language == "en" ? createDocumentSchema : createDocumentSchemaAr
    ),
    defaultValues,
  });
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <AddDocument
          ...
          error={
            errors.bank_account_letter?.message ||
            api_errors?.bank_account_letter
          }
          setValue={setValue}
          inputs={inputs}
        />
