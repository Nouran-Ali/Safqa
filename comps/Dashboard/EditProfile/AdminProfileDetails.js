...
import { ResetSuccess, getProfile, updateProfile } from "../../../store/slices/profileSlice";
import { useForm } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import { useEffect } from "react";
import { useTheme } from "next-themes";
import { SafqaInput, SafqaPhoneInput, SafqaSelect, SafqaTextArea } from "../Inputs";
import { adminUpdateProfileBusinessScehma } from "../../../lib/validations/en/profileBusinessScehma";
import { adminUpdateProfileBusinessScehmaAr } from "../../../lib/validations/ar/profileBusinessScehmaAr";
...
const AdminProfileDetails = ({ profile_business }) => {
  ...
  const { isLoading, api_errors, success } = useSelector(state => state.profile);
  ...
  const defaultValues = profile_business
  const country_active_banks = banks.filter(bank => bank?.country_id == profile_business?.country_id)
  ...
  const {
    ...
    resolver: yupResolver(language == 'en' ? adminUpdateProfileBusinessScehma : adminUpdateProfileBusinessScehmaAr),
    defaultValues,
  });
  ...
  useEffect(() => {
    if (success) {
      ...
      dispatch(getProfile(profile_business.id))
    }
  }, [dispatch, reset, success, profile_business]);
  ...
  //     setValue('currency_id', profile_business?.country_id)
  //     setValue('country_id', profile_business?.country_id)
  //     setValue('bank_id', profile_business?.bank_id)
  //     setValue('account_number', profile_business?.account_number)
  //     setValue('bank_account_name', profile_business?.bank_account_name)
  //     setValue('iban', profile_business?.iban)
  ...
  const ShowImgButton = () => {
    ...
    return (
      <button
        ...
        className={`dropdown-item ${theme == 'dark' ? "text-grey" : "text-dark"}`} >
        <VisibilityIcon />
  ...
  const MenuItems = (
    <Menu>
      <Menu.Item>
        <label className={`dropdown-item ${theme == 'dark' ? "text-grey" : "text-dark"}`} htmlFor="logo">
          <VerticalAlignTopRoundedIcon />
          <span className="ms-3 text-wrap">{logo?.[0]?.name || t("dashboard.change_picture")}</span>
        </label>
  ...
  // useEffect(() => {
  //   setValue('category_id', profile_business?.category_id)
  // },[profile_business?.category_id, setValue])
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className={`${styles.BusinessDetails} ${language == "ar" && "me-4"}`} dir={language == "ar" ? "rtl" : "ltr"}>
        <div className="row">
          ...
          <div className="col-xl-4 col-lg-4 col-md-6 col-sm-6 ">
            <div className="mb-3">
              ...
              <div className="position-relative mb-4">
                ...
                {errors.logo && <span className="text-danger fs-6">{errors.logo.message}</span>}
                {api_errors?.logo && <span className="text-danger fs-6">{api_errors.logo}</span>}
            ...
            <div className="mb-3">
              <SafqaTextArea
                ...
                register={register}
                error={errors.terms_and_conditions?.message || api_errors?.terms_and_conditions}
                rows="8"
              />
            </div>
            ...
            <div className="mb-3">
              <SafqaInput
                ...
                type="text"
                register={register}
                error={errors.account_number?.message || api_errors?.account_number}
                required
              />
            </div>
          </div>
          ...
          <div className="col-xl-4 col-lg-4 col-md-6 col-sm-6">
            ...
            <div className="mb-3">
              <SafqaSelect
                ...
                register={register}
                required
                error={errors.invoice_expiry_after_type_id?.message || api_errors?.invoice_expiry_after_type_id}
              />
            </div>
            ...
            <div className="mb-3">
              <SafqaInput
                ...
                type="number"
                register={register}
                error={errors.invoice_expiry_after_number?.message || api_errors?.invoice_expiry_after_number}
                required
              />
            </div>
            ...
            <div className="mb-3">
              <SafqaTextArea
                ...
                name="custom_sms_en"
                register={register}
                error={errors.custom_sms_en?.message || api_errors?.custom_sms_en}
              />
            </div>
            ...
            <div className="mb-3">
              <SafqaInput
                label={t("dashboard.bank_account_holder_name")}
                name="bank_account_name"
                type="text"
                register={register}
                error={errors.bank_account_name?.message || api_errors?.bank_account_name}
                required
              />
            </div>
          </div>
          ...
          <div className="col-xl-4 col-lg-4 col-md-12 col-sm-12">
            ...
            <div className="mb-3">
              <SafqaTextArea
                ...
                name="custom_sms_ar"
                register={register}
                error={errors.custom_sms_ar?.message || api_errors?.custom_sms_ar}
              />
            </div>
