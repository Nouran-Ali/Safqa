...
const InvoiceItemsUpdate = ({ register, setValue, errors, api_errors, defaultValues, inputs, control }) => {
...
const CreateNewItem = ({ append, inputs }) => {
  ...
  return fields.map(
    ({ id }, index) => (
      <div key={id} className={`d-flex mt-3 ${styles.row_width}`}>
        ...
        <div className={`${styles.w_16} ${styles.w_50} px-4`}>
          <button
            onClick={() => prductItems.length > 1 && remove(index)}
            type="button"
...
export default InvoiceItemsUpdate;
