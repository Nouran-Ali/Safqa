...
import { createPaymentMethodSchema } from "../../../lib/validations/en/paymentMethodSchema";
import { createPaymentMethodSchemaAr } from "../../../lib/validations/ar/paymentMethodSchemaAr";
import { createPaymentMethod, ResetSuccess } from "../../../store/slices/paymentMethodSlice";
...
const PaymentMethodInfo = () => {
  ...
  const { paymentMethodInfo, isLoading, api_errors, success } = useSelector(
  ...
  const {
    ...
    resolver: yupResolver(language == 'en' ? createPaymentMethodSchema : createPaymentMethodSchemaAr),
    defaultValues,
  });
  ...
  const onSubmit = (data) => dispatch(createPaymentMethod(data));
  ...
  return (
    <form className={`mt-2 mb-4`} onSubmit={handleSubmit(onSubmit)}>
      <div className={`rounded-2 ${theme == 'dark' ? styles.info_dark : styles.info}`} dir={language == "ar" ? "rtl" : "ltr"}>
        <p className="px-4 fs-5">{t("dashboard.paymentMethod_info")}</p>
        <hr />
