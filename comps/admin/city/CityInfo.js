...
import { createCity, ResetSuccess } from "../../../store/slices/citySlice";
...
const CityInfo = () => {
  ...
  const { cityInfo, isLoading, api_errors, success } = useSelector(
    (state) => state.city
  );
  ...
  const onSubmit = (data) => dispatch(createCity(data));
