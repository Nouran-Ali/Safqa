...
const PayLinkCheckoutComponent = ({ register, errors, api_errors, pay_link }) => {
    const [t, i18n] = useTranslation();
    const { language } = i18n;
    const { theme } = useTheme()

    return (
        <div className={`mb-4 rounded-2 ${theme == 'dark' ? styles.info_dark : styles.info} m-0 p-4`} dir={language == "ar" ? "rtl" : "ltr"}>
            <div className='d-flex justify-content-between'>
                <p className="px-4 fs-5">{t("dashboard.customer_info")}</p>
                <p className="px-4 fs-5">{t('total')} : <span className='fw-bold'>
                    {pay_link?.payment_amount && formatNumber(+pay_link.payment_amount)} ({pay_link?.currency?.short_currency})
                </span>
                </p>
            </div>
            <hr />
...
export default PayLinkCheckoutComponent